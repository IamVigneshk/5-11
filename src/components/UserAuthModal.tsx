...
// Pre-hashed password for "venom"
const HASHED_USER_PASSWORD = '$2a$10$YG7.WjP6JJwj8iR.F.Wm6.q2C9XG9U3tYLTXuGK9mX69k3M1cdHbK';
...
export default function UserAuthModal({ isOpen, onClose, onSuccess }: UserAuthModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (username === 'venom') {
      const isPasswordValid = await bcrypt.compare(password, HASHED_USER_PASSWORD);
      if (isPasswordValid) {
        onSuccess();
        onClose();
      } else {
